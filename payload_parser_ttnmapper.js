...
// Anpassungen von Bastian Hodapp für Funktionalität mit TTNMapper
...
function Decoder(bytes, port) {
    ...
    while (index < payload.length) {
        ...
        if ((next_field & 0x0f) == POWER_ON) {
            ...
            // This path was modified to put the GPS response info on the decoded payload top-level
            decoded.reason = reason_as_str(next_field);
            decoded.gnss_info = "Data_DD";

            decoded.latitude =
                parseFloat(uint32(payload.slice(index, index + 4))) / 1000000;
            index = index + 4;
            decoded.longitude =
                parseFloat(uint32(payload.slice(index, index + 4))) / 1000000;
            index = index + 4;
            decoded.altitude = (payload[index] << 8) + payload[index + 1];
            index += 2;
            decoded.hdop = decompress_dop(payload.slice(index, index + 2));
            index = index + 2;
            decoded.ttf = payload.slice(index, index + 1) * 2; // resolution 2 sec
            index = index + 1;

            if ((decoded.altitude & 0x8000) > 0) {
                decoded.altitude = decoded.altitude - 0x10000;
            }
        } else if ((next_field & 0x0f) == GNSS_TIMEOUT) {
            decoded.reason = reason_as_str(next_field);
            decoded.gnss_info = "Timeout";

            decoded.sat_in_view = payload[index++];
            decoded.gnss_active_time = payload[index++];
            ...
            config.application = {
                mode: payload[index],
                app_cycle_s:
                    ((payload[index + 1] << 8) | payload[index + 2]) * 60,
            };
            index += 3;
            config.algorithm = {
                slot_cnt: payload[index],
                slot_dur: (payload[index + 1] << 8) | payload[index + 2],
            };
            index += 3;
            config.inactivity = {
                threshold: payload[index],
                duration: (payload[index + 1] << 8) | payload[index + 2],
            };
            index += 3;
            config.activity = {
                threshold: payload[index],
                samples: payload[index + 1],
            };
            index += 2;
            config.qos = {
                mode: payload[index],
                utc_offset: payload[index + 1],
                sunrise: {
                    hour: payload[index + 2],
                    minute: payload[index + 3],
                },
                sunset: {
                    hour: payload[index + 4],
                    minute: payload[index + 5],
                },
            };
            index += 6;
            config.gnss_backup_mode = payload[index++] ? "enabled" : "disabled";
            config.application.nightly_app_cycle_s =
                ((payload[index] << 8) | payload[index + 1]) * 60;
            index += 2;
